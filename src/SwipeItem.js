...
const SwipeItem = React.memo(({ onSwipe }) => {
  ...
  React.useEffect(() => {
    ...
    const touchStart = (e) => {
      ...
      // 터치 시작 시, swipe_status를 touchstart로 변경
      // 그리고 터치 시작한 좌표를 기록
      // (중요! 그래야 터치 종료할 때 위치를 보고 왼쪽인지, 오른쪽인지 판별가능)
      swipe_status = "touchstart";
      target_classname = swipe_div.current.className;
      // console로 터치 이벤트가 시작될 때 좌표를 확인
    ...
    const touchEnd = (e) => {
      ...
      //  touchEnd이벤트는 touches 대신, changedTouches가 있음
      ...
      if (Math.abs(diff_x) > 50) {
        ...
        // 300 ms후 reset 
        // 이 300ms는 props로 받아서 처리
        // props로 받아온, 콜백 함수도 여기서 처리
        window.setTimeout(() => {
          reset();
          onSwipe(direct);
        }, 300);
