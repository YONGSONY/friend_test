...
export const addRankFB = (rank_info) => {
  return function (dispatch) {
    ...
    rank_db.add(rank_data).then((doc) => {
      ...
      console.log(doc.id);
      // id를 추가
      // current는 여기서 추가, 그래야 내가 한 것만 하이라이트를 줄 수 있음 (db에 current가 true로 들어가면 안됨)
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case "rank/GET_RANK": {
      ...
      // 리덕스에 있던 데이터에 파이어베이스에서 가져온 데이터를 추가 다만, 같은 값이 있으면 안됨
      // 중복되지 않은 데이터만 추가
      // id가 같은 지 아닌 지로 데이터를 구분해서 추가

      // 일단 랭킹 데이터를 담을 변수를 만들고, 기존 리덕스 값을 가져다가 넣어줌
      let ranking_data = [...state.ranking];

      // 랭킹 데이터의 id 배열을 하나 생성
      ...
      const rank_data_fb = action.rank_list.filter((r, idx) => {
        // 가지고 온 값의 id가 리덕스에 있는 아이디 배열에 없으면 추가
        if(rank_ids.indexOf(r.id) === -1){
          // 배열에도 이렇게 스프레드 문법을 사용할 수 있음 (다른 방법으로 추가해도 됨)
